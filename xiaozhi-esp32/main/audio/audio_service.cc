        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            if (opus_decoder_ != nullptr) {
                ...
                esp_audio_dec_in_raw_t raw = {
                    .buffer = packet->packet_loss ? nullptr : (uint8_t *)(packet->payload.data()),
                    .len = packet->packet_loss ? 0 : (uint32_t)(packet->payload.size()),
                    .consumed = 0,
                    .frame_recover = packet->packet_loss ? ESP_AUDIO_DEC_RECOVERY_PLC : ESP_AUDIO_DEC_RECOVERY_NONE,
                };
...
void AudioService::PlayPcm(const int16_t* pcm, size_t samples) {
