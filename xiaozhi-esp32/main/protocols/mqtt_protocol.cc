...
namespace {
constexpr uint32_t kMaxPacketLossConcealmentFrames = 3;
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        if (sequence != expected_sequence) {
            ESP_LOGW(TAG, "Received audio packet with wrong sequence: %lu, expected: %lu", sequence, expected_sequence);
            if (sequence > expected_sequence) {
                missing_packets = sequence - expected_sequence;
            }
        }
        ...
        if (on_incoming_audio_ != nullptr) {
            uint32_t conceal_count = missing_packets;
            if (conceal_count > kMaxPacketLossConcealmentFrames) {
                conceal_count = kMaxPacketLossConcealmentFrames;
            }
            for (uint32_t i = 0; i < conceal_count; ++i) {
                ...
                uint32_t frames_before_current = conceal_count - i;
                uint32_t timestamp_offset = frames_before_current * server_frame_duration_;
                plc_packet->timestamp = timestamp > timestamp_offset ? timestamp - timestamp_offset : 0;
                plc_packet->packet_loss = true;
