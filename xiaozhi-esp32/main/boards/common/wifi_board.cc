...
std::string WifiBoard::GetDeviceStatusJson() {
    ...
    auto location = cJSON_CreateObject();
    McuUartStm32Data stm32_data;
    if (mcu_uart_get_latest_stm32_data(&stm32_data)) {
        cJSON_AddStringToObject(location, "source", "stm32_uart");
        cJSON_AddBoolToObject(location, "gps_signal", stm32_data.gps_signal);
        if (stm32_data.gps_signal) {
            cJSON_AddNumberToObject(location, "longitude", stm32_data.longitude);
            cJSON_AddNumberToObject(location, "latitude", stm32_data.latitude);
        } else {
            cJSON_AddStringToObject(location, "gps_status", stm32_data.gps_status.c_str());
        }
        cJSON_AddBoolToObject(location, "fall_alarm", stm32_data.fall_alarm);
        cJSON_AddStringToObject(location, "address_tool", "self.location.get_current");
    } else {
        cJSON_AddStringToObject(location, "source", "none");
        cJSON_AddStringToObject(location, "address_tool", "self.location.get_current");
    }
    cJSON_AddStringToObject(location, "note", "Do not infer physical location from WiFi SSID.");
